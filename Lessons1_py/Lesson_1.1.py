# bu print('' yoki "")ni ichiga malumot kiritiladi ekranga chiqarish uchun
# Misol:
print('Hello python code')
print('My name is Abdulaziz')
print("I'm 21 years old")

# print ichida hisob kitob qilsa ham bo'ladi
print(4+8)
print(84-47)
print(8*5)
print(96//3)
print(8475/4758)
print(4**4)

# print ichida hisob kitob qilsa ham bo'ladi
print(4+8)
print(84-47)
print(8*5)
print(96//3)
print(8475/4758)
print(4**4)

#endi o'zgaruvchilar bilan ishlaymiz
x = 8
y = "Azizbek"
print(x)
print(y)
# ko'rib turganingizdek buyerda 2ta bir xil o'zgaruvchi bor shulardan oxirgisi chiqadi
x = 4
x = "Sally"
print(x)

# hozir biz belgilar bilan tanishamiz
# bu "" <-- belgi string deyiladi
# bu % <-- bo'linmaning qoldig'ini olish
# va bu kabi +, -, *, **, //, / <-- belgilar arifmetik amalda qo'llaniladi
# yana comment belgisi qora rangda bo'ladi --> # kabi belgi

# Misollar
print("\"Nexia\", \"Tico\", 'Damas' ko'rganlar qilar havas")

#1 5 ning 4-darajasi
print('5 ning 4-darajasi', 5**4)

#2 22 ni 4 ga bo'lganda qancha qoldiq qoladi?
print("22 ni 4 ga bo'lganda qancha qoldiq", 22%4)

#3 Tomonlari 125 ga teng kvadratning yuzi va perimetrini toping
print("Tomonlari 125 ga teng kvadratning yuzi", 125*125, "ga, perimetri", 4*125, "teng")

#4 Diametri 12 ga teng bo'lgan doiraning yuzini toping
print('Diametri 12 ga teng bo\'lgan doiraning yuzi', 3.14*(12/2)**2, 'ga teng')

#5 Katetlari 6 va 7 bo'lgan to'g'ri burchakli uchburchakning gipotenuzasini toping
print("Katetlari 6 va 7 bo'lgan to'g'ri burchakli uchburchakning gipotenuzasi", (6**2+7**2)**(1/2))

